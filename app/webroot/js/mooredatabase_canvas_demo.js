(function() {
  ...
  addFlake = function() {
    ...
    if (flakeArray.length === maxFlakes) return clearInterval(flakeTimer);
  };
  ...
  Update = function() {
    var i, _ref, _results;
    _results = [];
    for (i = 0, _ref = flakeArray.length; 0 <= _ref ? i < _ref : i > _ref; 0 <= _ref ? i++ : i--) {
      if (flakeArray[i].y < context.canvas.height) {
        flakeArray[i].y += flakeArray[i].speed;
      }
      if (flakeArray[i].y >= context.canvas.height) flakeArray[i].y = startY;
